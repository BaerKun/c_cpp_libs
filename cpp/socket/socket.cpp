...
constexpr socklen_t SOCK_ADDR_SIZE{};
template<>
...
template<AddressFamily AF>
static inline sockaddr *getSockAddrBuff() noexcept {
...
static sockaddr *getSockAddr(const SocketAddress &addr) {
    if constexpr (AF == AddressFamily::IPv4) {
        const auto ipv4 = static_cast<sockaddr_in *>(std::malloc(SOCK_ADDR_SIZE<AF>));
    ...
    if constexpr (AF == AddressFamily::IPv6) {
        const auto ipv6 = static_cast<sockaddr_in6 *>(std::malloc(SOCK_ADDR_SIZE<AF>));
    ...
    const auto local = static_cast<sockaddr_un *>(std::malloc(SOCK_ADDR_SIZE<AF>));
...
static void sockSend(const int fd, const uint8_t *const ptr, const size_t size) {
    size_t sent = 0;
