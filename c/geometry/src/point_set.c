...
#define SORT_ELEM_TYPE Point2f
#define SORT_LESS_THAN(p, q) (p.x < q.x)

#include "sort.h"

#define STACK_ELEM_TYPE Point2f
...
static void convexHullHelper(const StackPtr stack, const Point2f p) {
  ...
  do {
    ...
    if (cross_vec(vec1, vec2) < 0) break;
...
static void ptsMaxDist_force(Point2f hullpts[], Point2f *outpts,
                             const int nhullpts) {
  float max = 0;
...
static float ptsMinDistHelper_force(const Point2f *ptsleft,
                                    const Point2f *ptsright, Point2f *outleft,
                                    Point2f *outright, float minsqd) {
...
static float ptsMinDistHelper(Point2f pts[], Point2f outpts[], int left,
                              int right) {
  if (right - left < 8)
    return ptsMinDistHelper_force(pts + left, pts + right, outpts + left,
                                  outpts + right, FLT_MAX);
  ...
  while (borderLeft > left &&
         pts[center].x - pts[borderLeft - 1].x < borderWidth)
    --borderLeft;

  while (borderRight < right &&
         pts[borderRight + 1].x - pts[center].x < borderWidth)
    ++borderRight;

  return ptsMinDistHelper_force(pts + borderLeft, pts + borderRight,
                                outpts + left, outpts + right, midMinSqrd);
}
...
void convexHull(Point2f pts[], Point2f hullpts[], int npts, int *nhullpts) {
  ...
  for (int i = 3; i < npts; ++i) convexHullHelper(&stack, pts[tmp[i].index]);
...
void convexHullDiameter(Point2f hullpts[], Point2f *outpts, int nhullpts) {
  ...
  Point2f p2 = hullpts[0], p3 = hullpts[1], p4 = hullpts[2];
...
void minEnclosingCircle(Point2f pts[], const int npts, Point2f *center,
                        float *radius) {
  if (npts <= 0) return;
  ...
  *radius = 0;
  for (int i = 1; i < npts; ++i) {
    ...
    if (dist2f(*center, pts[i]) <= *radius) continue;
    *center = midpt(*pts, pts[i]);
    *radius = dist2f(*center, *pts);
    for (int j = 1; j < i; ++j) {
      if (dist2f(*center, pts[j]) <= *radius) continue;
      *center = midpt(pts[i], pts[j]);
      *radius = dist2f(*center, pts[i]);

      // 遍历三点外接圆
      for (int k = 0; k < j; ++k) {
        if (dist2f(*center, pts[k]) <= *radius) continue;
