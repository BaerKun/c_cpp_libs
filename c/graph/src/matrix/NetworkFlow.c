...
NetworkPtr CopyNetwork(const NetworkPtr network) {
  ...
  memcpy(copyNetwork->edges, network->edges,
         network->capacity * MAX_VERTEX * sizeof(Edge));

  return copyNetwork;
}

static int BFS(const NetworkPtr network, const QueuePtr queue,
               const VertexId source, const VertexId target, VertexId path[],
  ...
  while (queue->front != queue->rear) {
    ...
    for (VertexId outgoingVertex = 0; outgoingVertex < vertexNum;
         outgoingVertex++) {
      if (edges[thisVertex][outgoingVertex].data.capacity &&
          path[outgoingVertex] == -1) {
        path[outgoingVertex] = thisVertex;

        if (edges[thisVertex][outgoingVertex].data.capacity <
            edges[path[thisVertex]][thisVertex].data.flow)
          edges[thisVertex][outgoingVertex].data.flow =
              edges[thisVertex][outgoingVertex].data.capacity;
        else
          edges[thisVertex][outgoingVertex].data.flow =
              edges[path[thisVertex]][thisVertex].data.flow;
...
FlowType EdmondKarpMaxFlow(const NetworkPtr network, const VertexId source,
                           const VertexId sink) {
  FlowType singlrPathFlow;
  Queue queue;
  ...
  while (BFS(residualNetwork, &queue, source, sink, path, &singlrPathFlow)) {
    ...
    for (VertexId thisVertex = sink; thisVertex != source;
         thisVertex = path[thisVertex]) {
      ...
      residualNetwork->edges[incomingVertex][thisVertex].data.capacity -=
          singlrPathFlow;
      residualNetwork->edges[thisVertex][incomingVertex].data.capacity +=
          singlrPathFlow;
    }
  }
