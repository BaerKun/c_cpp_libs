...
void allPairsWeightedPath(const GraphPtr graph) {
  WeightType(*distances)[MAX_VERTEX] =
      malloc(sizeof(WeightType) * graph->vertexNum * graph->vertexNum);
  ...
  for (VertexId middle = 0; middle < graph->vertexNum; middle++) {
    for (source = 0; source < graph->vertexNum; source++) {
      if (distances[source][middle] == WIGHT_MAX) continue;

      for (target = 0; target < graph->vertexNum; target++) {
        if (distances[middle][target] == WIGHT_MAX) continue;

        if (distances[source][middle] + distances[middle][target] <
