...
int chooseTarget(Path *head, int source, float totalTime, char *mask) {
  float sum = 0.f, *_moveTime = moveTime[source],
        *_trendency = trendency[source];
  int i;

  // 筛选所有可行路径
  for (i = 0; i < nplaces; ++i) {
    if (totalTime + _moveTime[i] + workTime[i] > MAX_WORK_TIME ||
        haveVisited(head, i)) {
      mask[i] = 0;
      continue;
    }
...
void update(Path *head, float totalTime) {
  Path *source = head, *target = head->next;
  ...
  while (target != NULL) {
    ...
    trendency[source->data][target->data] =
        alpha * pheromone[source->data][target->data] /
        moveTime[source->data][target->data];
    source = target;
    target = target->next;
  }
}
