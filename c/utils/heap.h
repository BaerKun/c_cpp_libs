...
static void heapPercolateDown(const HeapPtr heap, const int top,
                              const int last) {
...
static void buildHeap(const HeapPtr heap, HEAP_DATA_TYPE *const data,
                      const int size) {
  heap->data = data;
  heap->prev = data - 1;
  heap->capacity = heap->size = size;
  for (int i = size >> 1; i; --i) heapPercolateDown(heap, i, size);
}
