...
static int CooleyTukeyHelper(const int polynomial[], const int x,
                             const int start, const int end, const int step) {
  if (start + step >= end) return polynomial[start];
...
static Complex dftHelper(const double time[], const Complex rotationFactor,
                         const int start, const int end, const int step) {
  if (start + step >= end) return (Complex){time[start], .0};
  ...
  return complexAdd(dftHelper(time, rotationFactorSq, start, end, step << 1),
                    complexMul(dftHelper(time, rotationFactorSq, start + step,
                                         end, step << 1),
                               rotationFactor));
}
...
void dft(const double time[], Complex frequency[], const int size) {
  for (int i = 0; i < size; ++i)
    frequency[i] = dftHelper(
        time, complexFromArgAndMod(-2 * M_PI * i / size, 1.), 0, size, 1);
}

static Complex idftHelper(const Complex frequency[],
                          const Complex rotationFactor, const int start,
                          const int end, const int step) {
  if (start + step >= end) return frequency[start];

  const Complex rotationFactorSq = complexMul(rotationFactor, rotationFactor);

  return complexAdd(
      idftHelper(frequency, rotationFactorSq, start, end, step << 1),
      complexMul(
          idftHelper(frequency, rotationFactorSq, start + step, end, step << 1),
          rotationFactor));
}
...
void idft(const Complex frequency[], double time[], const int size) {
  for (int i = 0; i < size; ++i) {
    // clang-format off
    time[i] = idftHelper(frequency,
      complexFromArgAndMod(2 * M_PI * i / size, 1.),
          0, size, 1).real / size;
    // clang-format on
  }
}

static void dftConvHelper(const double a[], const double b[], double c[],
                          const int size) {
...
void dftConv(const double a[], const double b[], double c[], const int size_a,
             const int size_b) {
