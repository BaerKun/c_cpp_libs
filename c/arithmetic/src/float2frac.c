...
void float2fraction_shift(const float x, const int maxShift, int *numerator,
                          int *rightShift) {
  const int *px = (int *)&x;
...
void float2fraction_precise(float x, int *numerator, int *denominator,
                            const int maxDeno) {
  const int *const px = (int *)&x, exponent = ((*px & 0x7F800000) >> 23) - 127;
  ...
  int n = 0, d = 1, outNume = 0, outDeno = 1;
