...
void lllAdd(uint8_t *output, const uint8_t *input1, const uint8_t *input2,
            const uint32_t size) {
  uint16_t carry = 0;
...
void lllAdd32(uint8_t *output, const uint8_t *input1, const uint32_t input2,
              const uint32_t size) {
  uint64_t buffer = input2;
  uint8_t *outptr = output;
  const uint8_t *inptr = input1, *end = output + size;
...
void lllSub(uint8_t *output, const uint8_t *minuend, const uint8_t *subtracted,
            const uint32_t size) {
...
void lllMul(uint8_t *output, const uint8_t *input1, const uint8_t *input2,
            const uint32_t size) {
...
void lllMul32(uint8_t *output, const uint8_t *input1, const uint32_t input2,
              const uint32_t size) {
  uint8_t *outptr = output;
...
void lllDiv8(uint8_t *quotient, const uint8_t *dividend, const uint8_t divisor,
             uint8_t *remainder, const uint32_t size) {
  if (divisor == 0) {
  ...
  while (quot_ptr != quotient) {
    ...
    if (quotient != NULL) *quot_ptr = dividend_tmp / divisor;
    dividend_tmp = dividend_tmp % divisor;
  }

  if (remainder != NULL) *remainder = dividend_tmp;
}

void lllDiv(uint8_t *quotient, const uint8_t *dividend, const uint8_t *divisor,
            uint8_t *remainder, const uint32_t size) {
  ...
  if (divisor_size == 1) {
    if (remainder != NULL) lllSet(remainder, 0, size);
    if (quotient != NULL)
  ...
  if (quotient != NULL) lllSet(quotient, 0, size);
  if (dividend_size < divisor_size) return;
  ...
  for (uint8_t *ptr = quotient + dividend_size - divisor_size; ptr >= quotient;
       --ptr, --remainder_high) {
    dividend_16 = dividend_16 << 8 | *remainder_high;
    if (dividend_16 < *divisor_high) continue;

    uint8_t quotient_tmp =
        (dividend_16 << 8 | *(remainder_high - 1)) / divisor_16;
    if (quotient_tmp == 0) continue;
    ...
    if (lldComp(subtracted_buffer, remainder_low, divisor_size_plus) == 1) {
      lllAdd(subtracted_buffer, subtracted_buffer, divisor_complement,
             divisor_size_plus);
      --quotient_tmp;
    }

    if (quotient != NULL) *ptr = quotient_tmp;
  ...
  if (remainder == NULL) free(remainder_);
}
