...
void calcByteCount(const unsigned char *input, unsigned *byteCount,
                   unsigned size) {
  for (int i = 0; i < size; ++i) byteCount[input[i]]++;
}

TrieTreePtr getHuffmanCodeTree(unsigned byte[], unsigned byteCount[],
                               unsigned *nbyte) {
  int j = 0;
  for (int i = 0; i < 256; ++i) {
    if (byteCount[i] != 0) {
...
void getCodeAndDepthHelper(TrieTreeNodePtr node, unsigned *codeArr,
                           unsigned *widthArr, unsigned code, unsigned width) {
  ...
  getCodeAndDepthHelper(node->right, codeArr, widthArr, code | (1 << width),
                        width + 1);
}
...
void loadOutput(Transfer *transfer) {
  unsigned i = 0, j = 0, tmpwidth, totalWidth = transfer->restBitWidth;
...
void finput(FILE *readFile, Transfer *transfer) {
  while (!feof(readFile)) {
    transfer->insize =
        (unsigned)fread(transfer->input, 1, STEAM_SIZE, readFile);
...
void process(Transfer *transfer) {
  TrieTreePtr tree = getHuffmanCodeTree(transfer->helper1, transfer->helper2,
                                        &transfer->nbytes);
...
int main(int argc, char *argv[]) {
  char str[512];
  char *readFilePath = str, *writeFilePath = str + 256;
